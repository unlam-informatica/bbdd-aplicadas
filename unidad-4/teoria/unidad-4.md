---
layout: page
title: "Unidad 4 — Apunte completo"
permalink: /unidad-4/teoria/
---

[← Unidad 4](../)

- [NoSQL](#nosql)
  - [Características y motivación](#características-y-motivación)
  - [Tipos de bases de datos NoSQL](#tipos-de-bases-de-datos-nosql)
- [MongoDB](#mongodb)
  - [Documentos y colecciones](#documentos-y-colecciones)
  - [BSON](#bson)
- [MongoDB Query Language (MQL)](#mongodb-query-language-mql)
  - [CRUD](#crud)
  - [Aggregation Pipeline](#aggregation-pipeline)
- [Data Lakes y Data Warehouses](#data-lakes-y-data-warehouses)
  - [Diferencias clave](#diferencias-clave)
- [KDD — Knowledge Discovery in Databases](#kdd--knowledge-discovery-in-databases)
  - [Etapas del proceso KDD](#etapas-del-proceso-kdd)

---

# Unidad 4 — BD No Transaccionales (NoSQL)

## NoSQL

> Contenido en construcción.

### Características y motivación

### Tipos de bases de datos NoSQL

## MongoDB

> Contenido en construcción.

### Documentos y colecciones

### BSON

## MongoDB Query Language (MQL)

> Contenido en construcción.

### CRUD

### Aggregation Pipeline

## Data Lakes y Data Warehouses

> Contenido en construcción.

### Diferencias clave

## KDD — Knowledge Discovery in Databases

> Contenido en construcción.

### Etapas del proceso KDD
